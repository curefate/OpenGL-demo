...
void main()
{
    ...
    //FragColor = vec4(vec3(texture(pointLight[0].shadowMap, fs_in.FragPos - pointLight[0].position).r / 25.0), 1.0);
}
...
vec3 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir)
{
    ...
    vec3 diffuse = light.diffuse * diff * vec3(0.3);
    vec3 specular = light.specular * spec * vec3(0.05);
